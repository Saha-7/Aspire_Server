...
| PAGINATION
|--------------------------------------------------------------------------
| FIX: no protection previously existed against Shopify's collection
| sort re-ordering (or an edge-cache serving a different snapshot)
| between two separate page fetches, which can cause the same
| product(s) to legitimately appear on two consecutive pages. Track
| each page's product-URL fingerprint (same approach already used in
| vedant.js) and stop pagination if we see a page we've already
| scraped the contents of, instead of silently re-scraping it.
*/
...
function getNextPageUrl(html, currentUrl) {
  ...
  const currentPageLinks = parseProductLinks(html);
  const fingerprint = [...currentPageLinks].sort().join('|');
