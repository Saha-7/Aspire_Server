...
async function getInsights(
  ...
  // NOTE: category is intentionally NOT filtered in SQL. The dropdown's
  // option list has to be built from rows that match every OTHER filter,
  // otherwise picking a category collapses the list down to just that one
  // category. We apply the category filter in JS further down instead.
  if (alertType) {
    if (alertType === "PRICE") {
  ...
  // Dropdown options: every category that has alerts under the current
  // tab/search/threshold, computed BEFORE the category filter is applied so
  // the list stays complete while a category is selected.
