...
// ─────────────────────────────────────────────────────────────
// Single product scraper — shared core used by:
//   1. src/scraper/index.js  (full category run, loops over this)
//   2. src/api_server.js     (manual refresh button, calls this once)
//
// CHANGE (parallel scraping): scrapeAndSave() now accepts an optional
// runExclusive() function so the shared products_full.json write is
// serialized even when many products are being scraped at once.
// rebuildPriceFile() no longer runs per-product — the category runner
// calls it once at the end instead.
// ─────────────────────────────────────────────────────────────

const { fetchPage }     = require('./fetchPage');
const { appendProduct } = require('./fileHelpers');
...
async function scrapeAndSave(store, productUrl, fullOutputPath, priceOutputPath, runExclusive = (fn) => fn()) {
  const product = await scrapeProduct(store, productUrl);

  if (product?.name) {
    await runExclusive(() => appendProduct(fullOutputPath, product));
  }
