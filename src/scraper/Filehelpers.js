 ...
 *
 * FIX: previously pushed unconditionally, so if the same product URL
 * was ever handed to this function twice in one run (pagination
 * re-visiting a URL, a listing page re-sorting between fetches, etc.)
 * it recorded the product twice — the direct mechanism behind
 * duplicate SKU rows showing up in ScrapeRunStats/ScrapeRunSkuMatches
 * (e.g. vishal's Power Supply category scraping to ~2x its real count).
 * Now skips (and overwrites in place) if this URL was already saved
 * this run, instead of appending a second copy.
 */
function appendProduct(fullOutputPath, product) {
  const existing = readJson(fullOutputPath, []);

  const dupeIndex = existing.findIndex(p => p.url === product.url);

  if (dupeIndex !== -1) {
    existing[dupeIndex] = product; // keep the latest scrape of this URL, don't duplicate
  } else {
